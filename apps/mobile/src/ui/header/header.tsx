...
export const BackButton: FC<PressableDefaultProperties> = ({ className, ...properties }) => {
  const { goBack } = useTypedNavigation();
  return (
    <Pressable className={cn('py-2', className)} onPress={goBack} {...properties}>
...
export const Logo: FC<Omit<TextDefaultProperties, 'onPress'>> = ({ className, ...properties }) => (
  <Title
    size={'xxl'}
    weight='bold'
...
export const Button: FC<Pick<ButtonProperties, 'variant' | 'children' | 'onPress'>> = ({
  onPress,
  variant = 'foreground'
}) => (
  <BaseButton size='sm' variant={variant} className='my-2 mr-2 py-1.5' onPress={onPress}>
    Sign in
  </BaseButton>
);
export const Icon: FC<Omit<IconProperties, 'variant' | 'noPadding' | 'size'>> = ({
  icon: Icon,
  className,
  ...properties
}) => (
