...
import { reactions, type reactionsTitles } from '@/screens/reader/reactions'
...
export interface CreateReactionFormType {
	slug: string
...
export const validation: z.ZodType<
	Omit<CreateReactionFormType, 'reaction'> & {
		reaction: string
	}
> = z.object({
	slug: z.string(),
...
const CreateReaction = () => {
	const { params } = useTypedRoute<'CreateReaction'>()
	...
	const { control, reset, handleSubmit } = useForm<CreateReactionFormType>({
	...
	return (
		<View className='bg-background h-full justify-between'>
			...
			<View className=' border-bordered border-t-2 pt-2'>
				<Controller
					control={control}
					name={'reaction'}
					render={({
