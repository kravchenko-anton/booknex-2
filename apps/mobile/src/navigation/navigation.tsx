...
const Navigation: FC = () => {
  ...
  const checkRefreshToken = async (route: keyof TypeRootStackParameterListType) => {
  ...
  useEffect(() => {
    const listener = navReference.addListener('state', () => {
      const route = navReference.getCurrentRoute()?.name as keyof TypeRootStackParameterListType;
