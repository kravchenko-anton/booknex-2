...
const ProgressBar = ({ tintColor = Color.primary, trackTintColor = Color.bordered, indeterminate, progress = 0, visible = true, animatedValue, style, fillStyle, testID = 'progress-bar', ...rest }) => {
    const isWeb = Platform.OS === 'web';
    const { current: timer } = useRef(new Animated.Value(0));
    const { current: fade } = useRef(new Animated.Value(0));
    const passedAnimatedValue = useRef(animatedValue);
    const [width, setWidth] = useState(0);
    const [previousWidth, setPreviousWidth] = useState(0);
    const indeterminateAnimation = useRef(null);
    useEffect(() => {
    ...
    const startAnimation = useCallback(() => {
         ...
         */
        const externalAnimation = passedAnimatedValue.current !== undefined &&
    ...
    return (<View onLayout={onLayout} {...rest} accessible accessibilityRole='progressbar' accessibilityState={{ busy: visible }} style={isWeb ? styles.webContainer : null} testID={testID} accessibilityValue={indeterminate ? {} : { min: 0, max: 100, now: progress * 100 }}>
			<Animated.View style={[
            styles.container,
            { backgroundColor: trackTintColor, opacity: fade },
            style
        ]}>
				{width ? (<Animated.View testID={`${testID}-fill`} style={[
                styles.progressBar,
                {
                    width,
