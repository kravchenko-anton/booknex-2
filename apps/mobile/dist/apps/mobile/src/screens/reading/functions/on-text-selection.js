...
const textSelectionValidation = (selectedText) => {
    if (selectedText.length > textSElectionLimit)
        return errorToast('Selected text is too long');
    return true;
};
export const onTextSelection = async (event, removeAllSelection) => {
    if (event.nativeEvent.key === 'copy') {
        if (!event.nativeEvent.selectedText)
            return;
        if (!textSelectionValidation(event.nativeEvent.selectedText))
            return;
...
//# sourceMappingURL=on-text-selection.js.map