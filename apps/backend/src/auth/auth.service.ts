...
export class AuthService {
  ...
  async googleSign(dto: GoogleAuthDto) {
    ...
    if (!data?.sub) throw serverError(HttpStatus.BAD_REQUEST, AuthErrors.invalidGoogleToken);
  ...
  async refresh(refreshToken: string) {
    ...
    if (!result) throw serverError(HttpStatus.BAD_REQUEST, GlobalErrorsEnum.invalidValue);
  ...
  private async validateUser(dto: AuthDto) {
    ...
    if (!user?.password) throw serverError(HttpStatus.NOT_FOUND, AuthErrors.passwordOrEmailInvalid);
