...
export const parseBookTable = async (page: Page, url: string, pageToGo: number) => {
  await page
    ...
    return [...books].map((book, index) => {
      ...
      return {
        ...
        ratingAvg: ratingAvg?.textContent
          ? Number.parseFloat(ratingAvg.textContent.split('—')[0].replaceAll('avg rating', ''))
          : 2.5
      };
    });
  });
...
export const parseCurrentBook = async (page: Page, url: string) => {
  ...
  await page.waitForSelector(
    parseSelectors.genres + ' > span:nth-child(1) > span > a > .Button__labelItem'
  );
  ...
  const rating = await page.evaluate((selector) => {
    ...
    return ratingCount?.textContent
      ? Number.parseFloat(
          ratingCount.textContent.replaceAll('ratings', '').replaceAll(',', '').trim()
        )
      : 0;
