...
export class ReviewService {
  ...
  private async checkBookExist(id: number) {
    ...
    if (!book) throw serverError(HttpStatus.BAD_REQUEST, GlobalErrorsEnum.unknownError);
    return !!book;
  }
  ...
  private async checkUserExist(id: number) {
    ...
    if (!userExist) throw serverError(HttpStatus.BAD_REQUEST, AdminErrors.userNotFound);
    return !!userExist;
  }
}
