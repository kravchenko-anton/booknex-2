...
export class UserService {
  ...
  async library(id: number) {
    ...
    if (!library) throw serverError(HttpStatus.BAD_REQUEST, GlobalErrorsEnum.somethingWrong);
  ...
  async isSaved(userId: number, id: number) {
    ...
    if (!user) throw serverError(HttpStatus.BAD_REQUEST, GlobalErrorsEnum.somethingWrong);
  ...
  private async checkBookExist(id: number) {
    ...
    if (!book) throw serverError(HttpStatus.BAD_REQUEST, GlobalErrorsEnum.somethingWrong);
    return !!book;
  }
}
