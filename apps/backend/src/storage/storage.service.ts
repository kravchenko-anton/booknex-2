...
import { StorageFolderArray, StorageFolderEnum, type StorageFolderType } from './storage.types';
...
export class StorageService {
  ...
  async upload({
    ...
    await this.s3
      ...
      .catch(() => serverError(HttpStatus.BAD_REQUEST, GlobalErrorsEnum.invalidValue));
    Logger.log(`File ${filename} uploaded to ${folder} folder`, StorageService.name);
    return {
