...
export const saveTokensStorage = (data: { accessToken: string; refreshToken: string }) => {
...
export const getNewTokens = async () => {
  ...
  const { data: response } = await api.auth.refreshToken({ refreshToken }).catch((error) => {
    console.log('error in get New token', error);
    return error;
  });
  console.log('response', response, 'response.accessToken', response.accessToken);
