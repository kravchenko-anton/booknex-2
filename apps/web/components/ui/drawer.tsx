...
>(({ className, ...properties }, reference) => (
  <DrawerPrimitive.Overlay
    ref={reference}
    className={cn('bg-foreground border-bordered fixed inset-0 z-50', className)}
    {...properties}
  />
));
...
const DrawerHeader = ({ className, ...properties }: HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('grid gap-1.5 p-4 text-center sm:text-left', className)} {...properties} />
);
DrawerHeader.displayName = 'DrawerHeader';

const DrawerFooter = ({ className, ...properties }: HTMLAttributes<HTMLDivElement>) => (
