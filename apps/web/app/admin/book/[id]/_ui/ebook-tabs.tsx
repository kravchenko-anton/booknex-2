...
const BookOverview: FC<EbookInfoProperties> = ({ bookId, onSuccess }) => {
  ...
  const { mutateAsync: updateEbook, isLoading: updateEbookLoading } = useMutation({
    mutationKey: ['update-ebook'],
    mutationFn: async ({ id, payload }: { id: number; payload: PayloadEBook[] }) => {
    ...
    <Tabs defaultValue='preview' className=' mt-8 w-full'>
      ...
      <div className='mt-4 p-2'>
        ...
        <TabsContent value='edit'>
          <Editor defaultBooks={ebook} updateBooks={(value: PayloadEBook[]) => setBooks(value)} />
          <Button
            size='md'
