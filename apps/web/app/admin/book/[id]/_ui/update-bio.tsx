...
const UpdateBio: FC<UpdateBioProperties> = (properties) => {
  const { mutateAsync: updateBio, isLoading: updateBioLoading } = useMutation({
    ...
    mutationFn: ({ id, payload }: { id: number; payload: UpdateBookBioValidationType }) =>
    ...
    <div>
      <div className='mb-4 flex flex-wrap items-center justify-center  gap-2 md:flex-nowrap'>
        ...
        <div className='md:w-5/12'>
          ...
          <Field control={control} name='rating' type='number' max={5} min={1} icon={Star} />
        </div>
      </div>
      ...
      <div className={cn('', Object.keys(dirtyFields).length > 0 ? 'mt-4 flex gap-2' : 'hidden')}>
        <Button
          size='sm'
          variant='foreground'
          onClick={() => {
