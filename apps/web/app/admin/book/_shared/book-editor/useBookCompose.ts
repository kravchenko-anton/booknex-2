...
export const useBookCompose = (defaultBooks?: PayloadEBook[]) => {
  ...
  const moveChaptersToNewBook = ({ bookId, chapterId }: { bookId: number; chapterId: number }) => {
  ...
  const mergeContentWithTopCharacter = ({
    ...
    setBooks((books) => {
      ...
      return books.map((book) => {
        if (book.id === bookId) {
          const element = book.chapters.find((content) => content.id === topChapterId);
          const index = book.chapters.findIndex((content) => content.id === topChapterId);
          if (!element) {
  ...
  const updateBookTitle = ({ value, bookId }: { value: string; bookId: number }) => {
