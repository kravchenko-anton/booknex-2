 ...
 * @export
 */
export const EbookApiAxiosParamCreator = function (
	...
	return {
		...
		ebookBySlug: async (
			...
			const localVarPath = `/api/ebook/ebook/by-slug/{slug}`.replace(
				`{${'slug'}}`,
		 ...
		 * @throws {RequiredError}
		 */
		storedEbookBySlug: async (
			slug: string,
			options: RawAxiosRequestConfig = {}
		): Promise<RequestArgs> => {
			// verify required parameter 'slug' is not null or undefined
			assertParamExists('storedEbookBySlug', 'slug', slug)
			const localVarPath = `/api/ebook/admin/stored-ebook/{slug}`.replace(
				`{${'slug'}}`,
				encodeURIComponent(String(slug))
			)
			// use dummy base URL string because the URL constructor only accepts absolute URLs.
...
export const EbookApiFp = function (configuration?: Configuration) {
	...
	return {
		...
		async ebookBySlug(
			...
			(axios?: AxiosInstance, basePath?: string) => AxiosPromise<EbookOutput>
		> {
			const localVarAxiosArgs = await localVarAxiosParamCreator.ebookBySlug(
				slug,
				options
			)
			const localVarOperationServerIndex = configuration?.serverIndex ?? 0
			const localVarOperationServerBasePath =
				operationServerMap['EbookApi.ebookBySlug']?.[
		...
		async storedEbookBySlug(
			...
			(
				axios?: AxiosInstance,
				basePath?: string
			) => AxiosPromise<Array<StoredEBook>>
		> {
			const localVarAxiosArgs =
				await localVarAxiosParamCreator.storedEbookBySlug(slug, options)
			const localVarOperationServerIndex = configuration?.serverIndex ?? 0
			const localVarOperationServerBasePath =
				operationServerMap['EbookApi.storedEbookBySlug']?.[
					localVarOperationServerIndex
				]?.url
			return (axios, basePath) =>
				createRequestFunction(
...
export const EbookApiFactory = function (
	...
	return {
		...
		storedEbookBySlug(
			...
			return localVarFp
				.storedEbookBySlug(slug, options)
				.then(request => request(axios, basePath))
		}
	}
}
