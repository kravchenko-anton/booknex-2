...
export const BookApiAxiosParamCreator = function (
	...
	return {
		 ...
		 */
		adminInfoBySlug: async (
			slug: string,
			options: RawAxiosRequestConfig = {}
		): Promise<RequestArgs> => {
			// verify required parameter 'slug' is not null or undefined
			assertParamExists('adminInfoBySlug', 'slug', slug)
			const localVarPath = `/api/book/admin-info/by-slug/{slug}`.replace(
				`{${'slug'}}`,
		...
		infoBySlug: async (
			...
			const localVarPath = `/api/book/info/by-slug/{slug}`.replace(
				`{${'slug'}}`,
				encodeURIComponent(String(slug))
			)
			// use dummy base URL string because the URL constructor only accepts absolute URLs.
		...
		remove: async (
			...
			const localVarPath = `/api/book/admin/remove/{slug}`.replace(
				`{${'slug'}}`,
				encodeURIComponent(String(slug))
			)
			// use dummy base URL string because the URL constructor only accepts absolute URLs.
		...
		update: async (
			...
			const localVarPath = `/api/book/admin/update/{slug}`.replace(
				`{${'slug'}}`,
				encodeURIComponent(String(slug))
			)
			// use dummy base URL string because the URL constructor only accepts absolute URLs.
	...
	return {
		...
		async adminInfoBySlug(
			...
			(
				axios?: AxiosInstance,
				basePath?: string
			) => AxiosPromise<AdminBookInfoOutput>
		> {
			const localVarAxiosArgs = await localVarAxiosParamCreator.adminInfoBySlug(
				slug,
				options
			)
			const localVarOperationServerIndex = configuration?.serverIndex ?? 0
			const localVarOperationServerBasePath =
				operationServerMap['BookApi.adminInfoBySlug']?.[
		...
		async infoBySlug(
			...
			(axios?: AxiosInstance, basePath?: string) => AxiosPromise<BookInfoOutput>
		> {
			const localVarAxiosArgs = await localVarAxiosParamCreator.infoBySlug(
				slug,
				options
			)
			const localVarOperationServerIndex = configuration?.serverIndex ?? 0
			const localVarOperationServerBasePath =
				operationServerMap['BookApi.infoBySlug']?.[localVarOperationServerIndex]
					?.url
			return (axios, basePath) =>
				createRequestFunction(
					localVarAxiosArgs,
...
export const BookApiFactory = function (
	...
	return {
		 ...
		 * @throws {RequiredError}
		 */
		remove(slug: number, options?: any): AxiosPromise<void> {
			return localVarFp
				.remove(slug, options)
				.then(request => request(axios, basePath))
		},
		...
		update(
			...
			return localVarFp
				.update(slug, updateBookDto, options)
				.then(request => request(axios, basePath))
		}
	}
}
...
export class BookApi extends BaseAPI {
	 ...
	 * @memberof BookApi
	 */
	public remove(slug: number, options?: RawAxiosRequestConfig) {
		return BookApiFp(this.configuration)
			.remove(slug, options)
			.then(request => request(this.axios, this.basePath))
	}
